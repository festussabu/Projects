...
def add_to_calculation(symbol):
    global calculaltion
    calculaltion += str(symbol)
    text_result.delete(1.0, "end")
    text_result.insert(1.0, calculaltion)

def evaluate_calculation():
    global calculaltion
    try:
        calculaltion = str(eval(calculaltion))
        text_result.delete(1.0, "end")
        text_result.insert(1.0, calculaltion)
    except:
        clear_field()
...
clear_open = tk.Button(root, text="(", command=lambda: add_to_calculation("("), width=5, font=("Arial", 14))
clear_open.grid(row=5, column=1)
result_close = tk.Button(root, text=")", command=lambda: add_to_calculation(")"), width=5, font=("Arial", 14))
result_close.grid(row=5, column=3)
